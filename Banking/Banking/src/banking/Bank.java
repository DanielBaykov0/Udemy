...
public class Bank {
    ...
    public boolean listCustomers(String nameOfBranch, boolean printTransactions) {
        ...
        if(tempBranch != null && printTransactions) {
            System.out.println("banking.Customer details for branch " + tempBranch.getName());
            for(int i=0; i<tempBranch.getCustomers().size(); i++) {
                System.out.println("banking.Customer: " + tempBranch.getCustomers().get(i).getName() + "[" + (i+1) + "]");
